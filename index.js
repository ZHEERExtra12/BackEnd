...
db.connect((err) => {
  if (err) {
    console.error("Database connection error:", err); // Detailed error logging
  } else {
...
app.post("/register", async (req, res) => {
  ...
  if (!req.body || !username || !password || !passwordAgain) {
    return res.status(400).json({ message: "نابێ خانەکان بەتاڵ بن" });
  }

  if (password !== passwordAgain) {
    return res.status(400).json({ message: "ووشەی نهێنی وەکو یەک نیە" });
  }
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    if (rows.length === 0) {
      return res.status(404).json({ message: "ئەم هەژمارە بونی نیە" });
    }
...
app.post("/additon", async (req, res) => {
  ...
  if (Inputs.some((input) => !input)) {
    return res.status(400).json({ message: "نابێ هیچ خانەک بەتاڵ بێت" });
  }
...
app.post("/sell-item", async (req, res) => {
  try {
    ...
    await db.promise().query(insertQuery, [finalQuantity, finalTotalPrice, finalProfit]);
